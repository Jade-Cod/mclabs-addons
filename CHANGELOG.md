...
### Changed
- **The HUD editor's Widgets list folds related widgets into one row.** Seventeen widgets made for a rail tall enough to cover the part of the screen you were trying to place things on. Related widgets now share one collapsible row, which brings it down to eight: **Boosts** (Chum Bucket, server and personal boosters, Lab Wars revenue, rental mount), **Events** (mini-events, The Pit, Raid Mine, bounties), **Reminders** (dailies, votes) and **Gambling** (open coinflips, coinflip record). A group's tick switches everything in it at once, and a shut group still lights up when one of its widgets is selected, so clicking a widget on the screen never leaves you hunting for it in the list. Nothing about the widgets themselves is shared — each keeps its own position, size, colours and on/off state, in every profile, exactly as before.

...
### Fixed
- **Selling chems moves your prestige bars again.** MCLabs changed the message it prints after a sale. Selling a single raw chem now says `Earned 104 prestige progress for Cactium.`, with the amount in the line itself, while a sale that earns progress for several chems (compound chems, or more than one raw chem) still says `Earned prestige progress for Cactium and Potatium.` and keeps each chem's amount in the hover. The mod only recognised the older wording, so single raw chem sales were ignored and the bars only caught up when you ran `/prestige progress`. Both wordings are read now.

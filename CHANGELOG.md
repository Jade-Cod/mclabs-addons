...
### Changed
- **The HUD editor's Widgets list can fold related widgets into one row.** Seventeen widgets is a long rail, and it sits over the part of the screen you are trying to place things on. Open Coinflips and Coinflip Record now share a single **Gambling** row that rolls open when you want the two of them; its tick switches both at once. Nothing about the widgets themselves is shared — each keeps its own position, size, colours and on/off state, and both still appear in every profile exactly as before.

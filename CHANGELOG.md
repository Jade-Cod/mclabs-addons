...
- **The crate board shows where you are on the pity ladder.** MCLabs raises your odds of a Rare+ item with every crate you open and puts a rarity back to the start when it lands, and it publishes the whole table: every reward's lore ends *"Click to view odds of each roll"*, and that opens a paginated list of your chance on each one. A window now hangs beside the chamber with **Exceedingly Rare, Super Rare and Very Rare** on it — each one's chance on the roll about to happen, and how many keys have gone in since it last landed. The two are one apart on purpose: the drought reads nought the moment a rarity lands, while the chance beside it is already the next key's. The figures are the server's own: all three ladders were scraped for about a thousand consecutive rolls apiece, and the mod reproduces every one of the 3,258 published values exactly, to the third decimal the server itself prints, rather than approximating them. Which turned up something a single page of the menu hides — **Exceedingly Rare is three curves, not one.** It climbs hard for ten rolls, then barely moves for the next 240 (0.24% to 0.281%, which is why one page of it reads as a flat line), and at roll 251 exactly it switches to a far steeper ramp and starts paying. The counters are shared across every crate, as the menu itself says, and the three ladders are entirely independent of each other: only the rarity that actually lands goes back to roll #1, and unboxing an Exceedingly Rare leaves your Super Rare and Very Rare streaks exactly where they were. So a four-hundred-roll Exceedingly Rare drought survives every other rarity landing on top of it. **Open a reward's odds once per rarity** and the mod reads exactly where you are, then counts on from the server's own *"your Exceedingly Rare odds have been JACKED-UP"* line, which fires on every roll whether or not you watched the spin — so a crate opened with the board switched off still counts. The menu labels no roll as yours, but it draws one: a roll already spent is an opened crate and one still ahead is a closed one, and the first head past the opened ones is the roll about to happen. Which is read off the heads themselves rather than off any list of what they should look like, so a page the server draws differently is read as saying nothing instead of as the wrong number. Until you have opened the menu, that rarity shows a dash rather than a figure, because a count with nothing behind it would read exactly like the server's and not be one. **Crate Roll Boosters are counted too** — right-clicking one moves all three ladders forward by however many rolls the voucher was worth, which is read off the server's own line rather than from a table, so all four tiers of the voucher work without one ever having been seen. A booster is not a roll and is not counted as one: no key is spent, nothing is unboxed, and no drought resets. On a screen too narrow to fit the window beside the panel it is dropped rather than shrunk.
- **The crate board cannot spoil a roll, by construction.** The winner is not in any packet until the eighth cull lands, about five seconds in, so there is nothing to reveal early. The only thing predicted is *when* the next cull is due, and that only draws a bar. It runs entirely on the server's clock, so a laggy spin is a slow spin rather than a desynchronised one.
- **A voter crate's three-way choice tells you what each draw is worth.** A voter crate rolls three rewards and lets you keep one, and states no odds anywhere on that screen — the figures exist only on the menu you get from punching the crate, which by then is long gone. Both crates' tables ship with the mod, so every draw carries its chance and its `1 in N` from the first crate you ever open; punching a crate still re-reads it, so a table the server retunes corrects itself without waiting for an update. Anything timed also says how long it runs for, which the name leaves out: Enhanced Farming Access comes in sixty, ninety and a hundred-and-twenty minute versions all called exactly the same thing, and the duration is both what you need to know before choosing and the only thing separating two of them in the odds table. The three reels spin like a slot machine, each reward sliding down out of its window as the next drops in, and the last one sliding to a stop when the server stops sending. Hovering a draw shows the item's own tooltip, because a Deluxe Voter Sword's four enchantments are nowhere in its name and you are being asked to choose it, and the whole column is clickable rather than just the icon. The figures are stated and nothing is ranked: a $75,000 at 3% against a Mystery Crate Key at 1% is not an ordering the odds can settle, so the mod does not attempt one. Neither voter screen names its crate, so the crate is worked out from the rewards themselves — once per roll, off the first set of rewards that names one — and where that is genuinely ambiguous no figures are shown rather than the wrong crate's.
- **The Pit's Fireball Staff is on the cooldown widget.** The Pit's newest item joins Stormbreaker, Excalibur, Blink Boots and the rest, with its own blaze-rod icon and its six seconds counted down from the server's own actionbar figure. Switch it off with the others in the widget's settings.
- **The casino games are drawn as games.** `/double`, `/mines` and `/bj` are chest menus — a wheel shown as a nine-wide strip of stained glass, a minefield of clay, a hand of cards kept as lore on a brewing stand — with everything you actually need spread across 54 slots. The mod draws its own panel in the chest's place: Double²'s wheel as a wheel with the five labs, the chips, the pot and the countdown; Mines as a 5x5 grid at a readable size with the payout ladder beside it; BondJoules as a card table with real faces, the four lab symbols mapped onto the four suits. Every control forwards a genuine click to the slot you would have hit yourself, so the server only ever sees an ordinary click, and the chips invest on the spot because that is what the server does — there is no confirm step to add. **Switch the boards off in Mod Menu and the vanilla menus are exactly as they were**; nothing is replaced beyond the drawing.
- **The boards state the server's own figures, and mark the ones that are worked out.** Mines is told only two numbers — what cashing out pays now and what one more star pays — so those are shown as given and the rest of the ladder is derived and labelled as derived. Measured across a captured round, the game returns exactly 70% of the fair multiplier at every rung, which means cashing out and revealing again are worth the same; the board says so rather than recommending anything. Blackjack's payouts are all measured now, including the ordinary win that had never been seen (even money, against 3:2 for a dealt 21), so the betting screen states all four outcomes with no caveats left on it.
- **Double²'s wheel learns itself.** The strip on screen shows nine of the 25 segments, and the order is not the same from one session to the next, so nothing about it is assumed. The wheel fills itself in as the strip slides past during a spin, drawing the part it has not seen yet in grey and saying it is still learning. Reopening the menu can start it anywhere, which is a rotation rather than a different wheel, so it is matched against what is already known and only thrown away and relearned when the order really has changed.
- **The tiles turn over and the cards are dealt.** Mines tiles flip rather than appear, and a losing reveal cascades outward from the mine that caused it instead of arriving all at once. Cards slide in as they are dealt and the hole card turns over where it lies. All of it is keyed to what the server actually sent, so a menu re-send replays nothing.
- **Police prestige and the Patrol challenges now track.** Confiscating states its own figure — `Earned 3,832 progress (x1.13) in confiscating contraband` — so an arrest or a bounty chest moves your bars the moment the message lands, with no guessing at the rate. Run **`/prestige progress`** once to pick up your contraband tier; only the tier you are working toward is shown, because the server runs one counter behind all of them and four identical bars would be noise. The Patrol challenges are credited by where the contraband was collected: plain Patrol always, Boat and Mount Patrol from what you are riding, and the neighbourhoods from the server's own region outlines, so it still knows you are in Purple long after the boss bar has faded. Walking into Yellow raises Green's bar too — the regions genuinely nest — so an arrest there counts for both. A frisk that turns up nothing credits nothing, and anything the client cannot be sure of is left for the next `/mastery` to true up rather than guessed at.

### Changed
- **The HUD editor's Widgets list folds related widgets into one row.** Seventeen widgets made for a rail tall enough to cover the part of the screen you were trying to place things on. Related widgets now share one collapsible row, which brings it down to eight: **Boosts** (Chum Bucket, server and personal boosters, Lab Wars revenue, rental mount), **Events** (mini-events, The Pit, Raid Mine, bounties), **Reminders** (dailies, votes) and **Gambling** (open coinflips, coinflip record). The whole list is alphabetical now, with the widgets that belong to no group gathered at the top, so the rows you can act on directly are all together and anything you are looking for is where its name says it should be. A group's tick switches everything in it at once, and a shut group still lights up when one of its widgets is selected, so clicking a widget on the screen never leaves you hunting for it in the list. Nothing about the widgets themselves is shared — each keeps its own position, size, colours and on/off state, in every profile, exactly as before.
...
### Fixed
- **A menu whose contents arrived a tick late is no longer skipped.** Every GUI the mod reads passively — `/lw rates`, `/chems booster`, `/ch`, `/supplier`, `/prestige`, `/mastery`, the smuggler's satchel, `/fw`, a voter crate's odds — is read once when it opens and then left alone, because its lore is a snapshot and re-reading it every tick would freeze the countdowns in it. A menu arrives in two packets, though: one opens the screen and another fills it. They almost always land together, but when the second was a tick behind, the read happened against an empty container and the menu was never read again — silently, until you opened it a second time. The read now waits for the contents.
- **Killing a Pit mob with an ability item now counts toward your Kill challenges.** A Pit kill is announced by nothing, so the mod works out whose kill it was, and the only thing it had to go on was your own melee swings — which meant a mob finished off with a Fireball Staff or Stormbreaker died credited to nobody. The server broadcasts who it holds responsible for every hit, including the player who fired a projectile, so that is read now and the ability kills land. Melee is unchanged. A kill the server attributes to nobody still counts for nothing rather than being guessed at, and one dealt by the Possessed Armour you summoned is the armour's as far as the server is concerned, so those still wait for `/mastery`.
- **A bounty chest no longer advances the Patrol challenges.** Every confiscation posts `Earned N progress in confiscating contraband`, and a bounty chest posts it under `Bounty »` where a frisk posts it under `MCLPD »`. Both were credited to the Patrol Mastery challenges as well as to police prestige, so opening a chest on horseback moved Mount Patrol — progress for contraband taken off a player, from a chest that had no player in it. The patrols now only count a frisk. Bounty chests still earn police prestige, and still advance **Secure Bounties**, which is their own challenge and was never the one at fault. Anything a line's prefix does not identify credits no patrol rather than a guessed one, so your bars can only ever be behind, never invented; the next `/mastery` trues them up.
...
### Fixed
- **Selling a single raw chem now moves your prestige bars.** A sale that earns progress for just one raw chem prints the amount in the line itself (`Earned 104 prestige progress for Cactium.`), and its hover only says "Click to see progress". Prestige tracking has only ever understood the other form, `Earned prestige progress for Cactium and Potatium.` with each chem's amount in the hover, which is what compound chems and sales of several raw chems print. So single raw chem sales have gone uncounted since tracking arrived in 1.15.0, and the bars only caught up when you ran `/prestige progress`. Both forms are read now.
- **The Sunken Treasure row goes away when a wave is over.** The last barrel of a wave is announced as `has found the final treasure! All sunken treasures have been found!` instead of "0 left", so the row stayed at "1 left" until you restarted. That line clears it now, and a wave with no treasure news for an hour (the Fishing Weekend ending with barrels still hidden, say) disappears on its own.
- **`/fw` clears a leftover Sunken Treasure row during any weekend event.** It only recognised the Fishing Weekend menu, so opening `/fw` during a Farm Weekend did nothing. Any weekend menu with no treasure in it now clears the row.

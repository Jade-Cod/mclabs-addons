...
- **A voter crate's three-way choice says which draw is the rare one.** A voter crate rolls three rewards and lets you keep one, and states no odds anywhere on that screen — the figures exist only on the menu you get from punching the crate, which by then is long gone. Both crates' tables ship with the mod, so every draw carries its chance and its `1 in N` from the first crate you ever open, with the rarest of the three flagged; punching a crate still re-reads it, so a table the server retunes corrects itself without waiting for an update. Anything timed also says how long it runs for, which the name leaves out: Enhanced Farming Access comes in sixty, ninety and a hundred-and-twenty minute versions all called exactly the same thing, and the duration is both what you need to know before choosing and the only thing separating two of them in the odds table. It says *rarest* and never *best*: a $75,000 at 3% against a Mystery Crate Key at 1% is not a ranking the odds can settle. Neither voter screen names its crate, so the crate is worked out from the rewards themselves — once per roll, off the first set of rewards that names one — and where that is genuinely ambiguous no figures are shown rather than the wrong crate's. Hovering a draw shows the item's own tooltip, because a Deluxe Voter Sword's four enchantments are nowhere in its name and you are being asked to choose it. A draw the table has no figure for stops anything being flagged the rarest at all: a 5% marked as the rare one beside a 0.5% the mod knows nothing about is worse than no mark.

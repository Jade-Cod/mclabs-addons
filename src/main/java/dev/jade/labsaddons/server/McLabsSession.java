...
public final class McLabsSession {
	...
	/**
	 * Identity-cached: the server only swaps in a new {@link Component} when the title actually
	...
	private static void read(Minecraft client) {
		Component title = sidebarTitle(client);
	...
	private static Component sidebarTitle(Minecraft client) {
		if (client.level == null) {
			return null;
		}
		Objective sidebar = client.level.getScoreboard()

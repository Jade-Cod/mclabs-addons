...
public final class D2Screen extends CasinoPanel {
	...
	protected boolean looksLike(AbstractContainerMenu handler) {
		for (Lab lab : Lab.values()) {
			ItemStack stack = handler.slots.get(lab.pickSlot()).getItem();
			if (stack.isEmpty() || Lab.fromText(stack.getHoverName().getString()) != lab) {
				return false;
			}
		}
		return true;
	...
	protected void draw(GuiGraphicsExtractor context, Font font, List<SlotView> slots,
		...
		if (offset >= 0) {
			...
			if (!D2Ring.isComplete()) {
				...
				context.text(font, note, wheelCx - font.width(note) / 2,
	...
	private void topBar(GuiGraphicsExtractor context, Font font, D2State state, int accent) {
		context.text(font, "DOUBLE²", PAD, 5, 0xFFFFFFFF, false);
		String phase = switch (state.phase()) {
			case BETTING -> "INVESTING OPEN";
			case SPINNING -> "MARKET CLOSED";
			case SETTLED -> "ROUND SETTLED";
		};
		context.text(font, phase, PANEL_W - PAD - font.width(phase), 5,
	...
	private static HubDisplay hubDisplay(D2State state, Font font) {
	...
	private void panel(GuiGraphicsExtractor context, Font font, D2State state,
		...
		for (Lab lab : Lab.values()) {
			...
			context.text(font, lab.glyph(), x + 5, y + 2, lab.lift(), false);
			// The multiplier is the one permanent fact about a lab, so it always shows.
			String mult = lab.multiplierText();
			String name = font.plainSubstrByWidth(lab.shortName(), width - 21 - font.width(mult));
			context.text(font, name, x + 15, y + 2, picked || hot ? 0xFFFFFFFF : TEXT, false);
			context.text(font, mult, x + width - font.width(mult) - 3, y + 2,
		...
		context.text(font, "INVESTED", x, y, TEXT_DIM, false);
		String staked = state.hasBet()
				? money(state.betAmount()) + " " + state.betLab().name()
				: money(state.stake());
		context.text(font, staked, x + width - font.width(staked), y,
		...
		context.text(font, pot, x, y, TEXT, false);
		// Spell it out when there is room; a very large pot takes the room back.
		String who = state.investors() + " investing";
		if (font.width(pot) + font.width(who) + 6 > width) {
			who = state.investors() + " in";
		}
		context.text(font, who, x + width - font.width(who), y, TEXT_DIM, false);
		y += line + 2;
		...
		if (state.phase() == D2State.Phase.SETTLED && state.settledLab() != null) {
			...
			context.text(font, result, x, y, state.settledLab().lift(), false);
			D2Chat.Outcome outcome = D2Chat.lastOutcome();
			if (outcome != null) {
				...
				context.text(font, settle, x + width - font.width(settle), y,
						outcome.won() ? WIN : LOSS, false);
			}
		} else if (state.eolDrought() >= 0) {
			context.text(font, "EOL dry", x, y, TEXT_DIM, false);
			String rounds = state.eolDrought() + " rounds";
			context.text(font, rounds, x + width - font.width(rounds), y, TEXT_DIM, false);
		}
	}

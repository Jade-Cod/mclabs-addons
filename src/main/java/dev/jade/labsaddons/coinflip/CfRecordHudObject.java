...
public class CfRecordHudObject extends HudObject {
	...
	public int contentWidth(boolean preview) {
		...
		return rows().stream()
				.mapToInt(row -> font.width(row.label())
						+ (row.value().isEmpty() ? 0 : VALUE_GAP + font.width(row.value())))
				.max().orElse(0);
	}
	...
	protected void renderContent(GuiGraphicsExtractor context, boolean preview) {
		...
		for (Row row : rows()) {
			if (!row.label().isEmpty()) {
				context.text(font, Component.literal(row.label()), 0, y, row.labelColor(), true);
			}
			if (!row.value().isEmpty()) {
				context.text(font, Component.literal(row.value()),
						width - font.width(row.value()), y, row.valueColor(), true);
			}

...
public class CfOpenHudObject extends HudObject {
	...
	public boolean shouldRender() {
		return !CfChat.openFlips(Util.getMeasuringTimeMs()).isEmpty();
	}

	private List<Row> rows(boolean preview) {
		List<CfChat.OpenFlip> flips = CfChat.openFlips(Util.getMeasuringTimeMs());
	...
	public int contentWidth(boolean preview) {
		...
		return rows(preview).stream()
				.mapToInt(row -> font.getWidth(row.text())
						+ (row.id().isEmpty() ? 0 : ID_GAP + font.getWidth(row.id())))
				.max().orElse(0);
	}
	...
	protected void renderContent(DrawContext context, boolean preview) {
		...
		for (Row row : rows(preview)) {
			...
			if (!row.id().isEmpty()) {
				context.drawText(font, Text.literal(row.id()),
						width - font.getWidth(row.id()), y, DIM_COLOR, true);
			}

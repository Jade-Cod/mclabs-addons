...
public final class CfFlipBoard extends CasinoPanel {
	...
	public Landed takePendingResult() {
		...
		Landed result = Util.getMillis() - pendingAtMs <= PENDING_MS ? pending : null;
		pending = null;
		return result;
	}
	...
	protected void draw(GuiGraphicsExtractor context, Font font, List<SlotView> slots,
		...
		if (jolt != 0) {
			...
			context.pose().translate(0f, jolt);
		}
	...
	private void coin(GuiGraphicsExtractor context, Font font, float turns, int faceUp,
	...
	private void seats(GuiGraphicsExtractor context, Font font, int faceUp) {
	...
	private void seat(GuiGraphicsExtractor context, Font font, int x, String name, String face,
			boolean up, boolean rightAligned) {
		if (name != null) {
			PlayerFaceExtractor.extractRenderState(context, PlayerSkinCache.skin(name), x, SEAT_Y, SEAT_HEAD);
		} else {
			context.fill(x, SEAT_Y, x + SEAT_HEAD, SEAT_Y + SEAT_HEAD, TRACK);
		}
		String label = name == null ? "…" : font.plainSubstrByWidth(name, 58);
	...
	private static void drawAligned(GuiGraphicsExtractor context, Font font, String text, int x,
			int y, int color, boolean rightAligned) {
		if (text.isEmpty()) {
			return;
		}
		context.text(font, text, rightAligned ? x - font.width(text) : x, y, color,
				false);
	}

	// --- the money ----------------------------------------------------------

	private void stakes(GuiGraphicsExtractor context, Font font) {
		int width = PANEL_W - PAD * 2;
		if (wagerCents <= 0L) {
			row(context, font, PAD, POT_Y, width, "pot", "—", TEXT_FAINT, TEXT_FAINT);
			context.text(font, "this screen never states the wager", PAD, TAX_Y,
					TEXT_FAINT, false);
			return;
		}
	...
	private void outcomes(GuiGraphicsExtractor context, Font font) {
		if (wagerCents <= 0L) {
			return;
		}
		int width = PANEL_W - PAD * 2;
	...
	private void banner(GuiGraphicsExtractor context, Font font) {
		boolean won = result == CfFlipReader.Result.WON;
		String headline = won ? "YOU WON" : "YOU LOST";
		context.pose().pushMatrix();
		context.pose().translate(PAD, BANNER_Y);
		context.pose().scale(BANNER_SCALE, BANNER_SCALE);
		context.text(font, headline, 0, 0, won ? WIN : LOSS, false);
		context.pose().popMatrix();
		...
		context.text(font, amount, PANEL_W - PAD - font.width(amount),
				BANNER_Y + font.lineHeight, won ? WIN : LOSS, false);
		String detail = won
				? "collected " + Money.format(CfOdds.winReturnCents(wagerCents))
				: "the stake is gone";
		context.text(font, detail, PAD, BANNER_SUB_Y, TEXT_DIM, false);
	}

	/** One line under the coin saying what it is doing. Centred, because the coin is. */
	private void state(GuiGraphicsExtractor context, Font font, long now) {
		long elapsed = now - startedAtMs;
		String text;
		int color;
		if (result != CfFlipReader.Result.RUNNING) {
		...
		if (!text.isEmpty()) {
			context.text(font, text, COIN_CX - font.width(text) / 2, STATE_Y, color,
					false);
		}
	}
	...
	private static boolean isHead(AbstractContainerMenu handler, int slot) {
		ItemStack stack = stackAt(handler, slot);
		return stack != null && stack.getItem() == Items.PLAYER_HEAD;
	}

	private static boolean isBlankPane(AbstractContainerMenu handler, int slot) {
		ItemStack stack = stackAt(handler, slot);
		return stack != null && stack.getHoverName().getString().isBlank();
	}

	private static ItemStack stackAt(AbstractContainerMenu handler, int slot) {

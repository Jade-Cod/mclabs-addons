 ...
 */
final class GlowRenderState implements GuiElementRenderState {
	...
	GlowRenderState(Matrix3x2fc pose, float cx, float cy, float radius, int colour,
		...
		this.bounds = new ScreenRectangle(minX, minY, size, size).transformMaxBounds(this.pose);
	}
	...
	public void buildVertices(VertexConsumer consumer) {
		...
		for (int band = 0; band < BANDS; band++) {
			...
			for (int slice = 0; slice < SLICES; slice++) {
				...
				consumer.addVertexWith2DPose(pose, cx + outer * cos0, cy + outer * sin0).setColor(outerColour);
				consumer.addVertexWith2DPose(pose, cx + inner * cos0, cy + inner * sin0).setColor(innerColour);
				consumer.addVertexWith2DPose(pose, cx + inner * cos1, cy + inner * sin1).setColor(innerColour);
				consumer.addVertexWith2DPose(pose, cx + outer * cos1, cy + outer * sin1).setColor(outerColour);
			}
		}
	}

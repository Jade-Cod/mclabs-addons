 ...
 * {@code GuiGraphicsExtractor} has no primitive for, so it goes in as an element of its own — see
...
final class CrateChamber {
	 ...
	 */
	static void glow(GuiGraphicsExtractor context, float cx, float cy, float radius, int colour,
			float intensity) {
		...
		GuiElements.submit(context, new GlowRenderState(context.pose(), cx, cy, radius,
				colour, layerAlpha));
	}
	 ...
	 */
	static void vignette(GuiGraphicsExtractor context, int x, int y, int w, int h, float intensity) {
	 ...
	 */
	static void mote(GuiGraphicsExtractor context, ItemStack stack, int colour, float cx,
		...
		context.pose().translate(cx - ICON * iconScale / 2f, cy - ICON * iconScale / 2f);
	...
	static void centred(GuiGraphicsExtractor context, Font font, String text, int cx, int y,
			int colour) {
		if (text == null || text.isEmpty()) {
			return;
		}
		context.text(font, text, cx - font.width(text) / 2, y, colour, false);
	}

	private static void outline(GuiGraphicsExtractor context, int x, int y, int w, int h, int colour) {

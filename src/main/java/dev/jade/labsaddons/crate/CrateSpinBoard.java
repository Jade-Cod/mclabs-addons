...
public final class CrateSpinBoard extends CasinoPanel {
	 ...
	 */
	private void pity(GuiGraphicsExtractor context, Font font) {
		...
		context.text(font, "YOUR ODDS", x + PAD, 5, TEXT_DIM, false);
		...
		for (CrateRarity rarity : CratePity.TRACKED) {
			...
			context.text(font, font.plainSubstrByWidth(rarity.label(), width), x + PAD, y,
					rarity.color(), false);
			if (roll <= 0) {
				context.text(font, PITY_UNKNOWN, x + PAD, y + PITY_LINE, TEXT_FAINT, false);
			} else {
		...
		if (anyUnknown) {
			caption(context, font, x + PAD, y,
					font.plainSubstrByWidth("open its odds once", width));
		}
	}
	...
	private List<CrateSpin.Cell> read() {
		...
		for (int column = 0; column < CrateSpin.COLUMNS; column++) {
			...
			cells.add(new CrateSpin.Cell(alive, CrateRarity.fromPane(path(stackAt(column))),
					alive ? candidate.getHoverName().getString() : ""));
		}
		return cells;
	}
	...
	private void candidates(GuiGraphicsExtractor context, long now) {
	 ...
	 */
	private void hoveredTooltip(GuiGraphicsExtractor context, Font font, long now) {
		for (int column = 0; column < CrateSpin.COLUMNS; column++) {
			if (!spin.column(column).alive()) {
				continue;
			}
	...
	private void candidate(GuiGraphicsExtractor context, int column, float x, float y, float scale,
	...
	private void flash(GuiGraphicsExtractor context, long now, CrateRarity won) {
	 ...
	 */
	private void name(GuiGraphicsExtractor context, Font font, long now) {
		CrateSpin.Column winner = spin.winner();
	...
	private static ItemStack stackOf(AbstractContainerMenu handler, int slot) {
		if (slot < 0 || slot >= handler.slots.size()) {
			return ItemStack.EMPTY;
		}
		return handler.slots.get(slot).getItem();
	}

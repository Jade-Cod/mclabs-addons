 ...
 * mob's name as it dies, and {@link LivingEntity#isDeadOrDying()} flips once its health
 * hits zero. We remember each matched mob by entity id the first time its tag is
 * readable — so the mid-fight heart bar, which no longer spells the mob name, cannot
 * lose it — then credit the kill exactly once when it dies (the death animation lingers
 * ~20 ticks, all reporting {@code isDeadOrDying()}).
 *
...
public final class MasteryKillTracker {
	...
	public static boolean tick(ClientLevel world) {
		...
		for (Entity entity : world.entitiesForRendering()) {
			...
			advanced |= observe(id, name == null ? null : name.getString(), mob.isDeadOrDying(), targets, nowMs);
		}

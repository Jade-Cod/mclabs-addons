...
public class HudEditScreen extends Screen {
	...
	private void buildInspector(HudObject widget) {
		...
		if (alarm != null) {
			inspectorChild(Button.builder(
							groupHeaderLabel(Component.translatable(alarm.lang()), alarmExpanded),
							b -> {
			...
			if (alarmExpanded) {
				inspectorChild(CycleButton.onOffBuilder(alarm.enabled()).create(
						innerX, y, innerW, EditorTheme.ROW,
						Component.translatable(alarm.lang() + ".enabled"),
				...
				inspectorChild(CycleButton.builder(RunnerAlarm::soundLabel, alarm.sound())
						.withValues(RunnerAlarm.SOUND_IDS)
						.create(innerX, y, innerW, EditorTheme.ROW,
								Component.translatable(alarm.lang() + ".sound"),
	...
	private static final class ThresholdSlider extends AbstractSliderButton {
		...
		ThresholdSlider(int x, int y, int width, int height, int initial, int min, int max,
			...
			super(x, y, width, height, Component.empty(), (Math.clamp(initial, min, max) - min) / (double) (max - min));
			this.min = min;
			this.max = max;
		...
		protected void updateMessage() {
			setMessage(Component.translatable(labelKey, current()));
		}

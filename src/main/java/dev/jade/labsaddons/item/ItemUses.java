 ...
 * flattening lore {@link Text} to a String, 296 ms running the regex over it, and 128 ms
 ...
 * {@link LoreComponent} and {@link NbtComponent} are immutable, and a stack whose data changes
 ...
 * on purpose: {@code LoreComponent} is a record, so its {@code equals} and {@code hashCode}
 * walk the whole text tree, which is the cost being avoided.
 *
 * <p>Read from the render thread only, via {@code DrawContextMixin}; a plain map is enough
...
public final class ItemUses {
	...
	/** The one accessor {@link NbtComponent} offers is a deep copy, so this runs once. */
	private static int readUses(NbtComponent component) {
		reads++;
		NbtCompound data = component.copyNbt();
		return data.contains("uses") ? data.getInt("uses", NONE) : NONE;
	}

	/** Flattens each lore line and looks for the count. Runs once per distinct lore. */
	static int scan(LoreComponent lore) {
		reads++;
	...
	static int charges(LoreComponent lore) {
		Integer known = ANSWERS.get(lore);
		return known != null ? known : remember(lore, scan(lore));
	}

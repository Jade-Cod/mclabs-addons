...
public abstract class CasinoPanel {
	 ...
	 */
	protected abstract boolean looksLike(AbstractContainerMenu handler);
	 ...
	 */
	protected abstract void draw(GuiGraphicsExtractor context, Font font,
	 ...
	 */
	public final boolean recognises(AbstractContainerScreen<?> screen) {
		return owns(screen) || (mayBe(screen.getMenu())
	 ...
	 */
	private boolean mayBe(AbstractContainerMenu handler) {
	...
	public final boolean owns(AbstractContainerScreen<?> screen) {
		return container.pinnedTo(screen.getMenu().containerId);
	}
	 ...
	 */
	public final boolean render(AbstractContainerScreen<?> screen, GuiGraphicsExtractor context,
		...
		ContainerHold.Result held = container.offer(read, fresh, syncId,
				holdKey(title), Util.getMillis(), STALE_MS, REOPEN_MS);
		...
		draw(context, font, slots, title,
				(float) client.getWindow().getGuiScale() * panelScale);
	 ...
	 */
	public final boolean mouseClicked(AbstractContainerScreen<?> screen, double mouseX, double mouseY,
			int button) {
		if (!owns(screen)) {
			return false;
		}
	...
	public final boolean mouseScrolled(AbstractContainerScreen<?> screen, double amount) {
		if (!owns(screen)) {
			return false;
		}
		onScroll(amount);
	...
	/**
	 * The click packet {@code handleContainerInput} would send, without it first playing the
	 * click out on the client.
	 *
	 ...
	 */
	private void sendClick(AbstractContainerScreen<?> screen, int slot, ContainerInput action) {
		Minecraft client = Minecraft.getInstance();
		...
		network.send(new ServerboundContainerClickPacket(menu.containerId, menu.getStateId(),
				(short) slot, (byte) 0, action, new Int2ObjectOpenHashMap<>(),
				HashedStack.create(menu.getCarried(), network.decoratedHashOpsGenenerator())));
	}

	private List<SlotView> slotViews(AbstractContainerMenu handler) {
	 ...
	 * test uses, because {@code GuiGraphicsExtractor} defers tooltips to the end of the frame and keeps
	 * no copy of the matrix — by the time it draws, the panel's transform is long popped.
	 */
	protected final void tooltip(GuiGraphicsExtractor context, Font font, ItemStack stack) {
	...
	protected final void clickable(int x, int y, int w, int h, int slot) {
		hits.add(new Hit(x, y, w, h, slot, ContainerInput.PICKUP));
	}
	...
	protected final void header(GuiGraphicsExtractor context, Font font, String title,
			String status, int statusColor) {
		context.text(font, title, PAD, 5, 0xFFFFFFFF, false);
		if (status != null && !status.isEmpty()) {
			String fit = font.plainSubstrByWidth(status, PANEL_W - PAD * 2 - font.width(title) - 6);
			context.text(font, fit, PANEL_W - PAD - font.width(fit), 5,
					statusColor, false);
		}
	 ...
	 */
	protected final void button(GuiGraphicsExtractor context, Font font, int x, int y,
			int w, int h, String label, int slot, int accent) {
		button(context, font, x, y, w, h, label, slot, accent, ContainerInput.PICKUP);
	}

	/** The same, for a control the server only answers to a shift-click on. */
	protected final void button(GuiGraphicsExtractor context, Font font, int x, int y,
			int w, int h, String label, int slot, int accent, ContainerInput action) {
		boolean live = slot >= 0;
		...
		context.text(font, fit, x + (w - font.width(fit)) / 2,
	...
	protected final void row(GuiGraphicsExtractor context, Font font, int x, int y,
			int width, String label, String value, int labelColor, int valueColor) {
		context.text(font, label, x, y, labelColor, false);
		if (value != null && !value.isEmpty()) {
			String fit = font.plainSubstrByWidth(value, width - font.width(label) - 4);
			context.text(font, fit, x + width - font.width(fit), y, valueColor, false);
		}
	}

	/** A small capitalised section label, in the editor's voice. */
	protected final void caption(GuiGraphicsExtractor context, Font font, int x, int y,
			String text) {
		context.text(font, text, x, y, TEXT_FAINT, false);
	}

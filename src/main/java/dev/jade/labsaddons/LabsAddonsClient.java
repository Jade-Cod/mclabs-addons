...
public class LabsAddonsClient implements ClientModInitializer {
	...
	private static net.minecraft.client.gui.screens.Screen scrapeWaitScreen;
	...
	public void onInitializeClient() {
		...
		ClientSendMessageEvents.COMMAND.register(command -> {
			...
			// ClientPacketListenerMixin — but both end up in the same place.
			CfChat.onCommandSent(sent, Util.getMillis());
		});
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			if (client.gui.screen() == null) {
				...
				if (landed != null) {
					client.setScreenAndShow(new CfResultScreen(landed));
				}
			}
	 ...
	 */
	private static boolean readyToScrape(AbstractContainerScreen<?> screen) {
		...
		for (int slot = 0; slot < slots.size() - 36; slot++) {
			if (!slots.get(slot).getItem().isEmpty()) {
				return true;
			}
		}

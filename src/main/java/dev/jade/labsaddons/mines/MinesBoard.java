...
public final class MinesBoard extends CasinoPanel {
	...
	private void grid(GuiGraphicsExtractor context, MinesState state, int accent) {
		long now = Util.getMillis();
		for (int row = 0; row < 5; row++) {
			for (int column = 0; column < 5; column++) {
				...
				if (squeeze < 1f) {
					...
					context.pose().translate(x + TILE / 2f, y + TILE / 2f);
					context.pose().scale(Math.max(0.02f, squeeze), 1f);
					context.pose().translate(-(x + TILE / 2f), -(y + TILE / 2f));
				}
	...
	private void face(GuiGraphicsExtractor context, MinesState.Tile tile, int index, int x, int y,
	...
	private void odds(GuiGraphicsExtractor context, Font font, MinesState state) {
		int y = GRID_Y + GRID + 4;
		if (state.blown()) {
			context.text(font, "mine hit — grid revealed", GRID_X, y, LOSS, false);
			return;
		}
		...
		context.text(font, "next tile", GRID_X, y, TEXT_DIM, false);
		int x = GRID_X + font.width("next tile ");
		context.text(font, chance, x, y, LOSS, false);
		context.text(font, " mine", x + font.width(chance), y, TEXT_DIM, false);
	}

	private void rail(GuiGraphicsExtractor context, Font font, MinesState state, int accent) {
		int y = CONTENT_Y + 2;
		caption(context, font, RAIL_X, y, "STAKE");
		y += font.lineHeight;
		context.text(font, state.stakeText(), RAIL_X, y, TEXT, false);
		y += font.lineHeight + 4;
	 ...
	 */
	private void ladder(GuiGraphicsExtractor context, Font font, MinesState state, int top,
			int accent) {
		...
		for (int i = 0; i < LADDER_ROWS; i++) {
			...
			context.text(font, index, RAIL_X, y, current ? accent : TEXT_FAINT, false);

			String mult = MinesOdds.multiplierText(state.mines(), stars);
			int multW = font.width(mult);
			context.text(font, mult, RAIL_X + RAIL_W - multW, y, color, false);
	...
	private void cashOut(GuiGraphicsExtractor context, Font font, MinesState state, int y) {
		...
		context.text(font, label, RAIL_X + (RAIL_W - font.width(label)) / 2, y + 3,
		...
		if (!amount.isEmpty()) {
			String fit = font.plainSubstrByWidth(amount, RAIL_W - 4);
			context.text(font, fit, RAIL_X + (RAIL_W - font.width(fit)) / 2, y + 12,
	 ...
	 */
	private void result(GuiGraphicsExtractor context, Font font, MinesState state, int y) {

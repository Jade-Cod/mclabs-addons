...
public class RaidMineHudObject extends HudObject {
	...
	public EditorAction editorAction() {
		return new EditorAction(Text.translatable("labsaddons.hud.raid_mine.reset"),
	...
	public Text toggleGroupsLabel() {
		return RaidMineSession.knownCodes().isEmpty()
				? null
				: Text.translatable("labsaddons.hud.raid_mine.resources");
	}
	...
	private int nameWidth(List<RaidMineSession.Row> rows) {
		...
		for (RaidMineSession.Row row : rows) {
			width = Math.max(width, font().getWidth(RaidMineResources.name(row.code())));
		}
		return width;
	}
	...
	protected void renderContent(DrawContext context, boolean preview) {
		...
		for (RaidMineSession.Row row : rows) {
			...
			context.drawText(font, Text.literal(RaidMineResources.name(row.code())),
					0, y, row.color() | 0xFF000000, true);
			String value = amount(row.total());
			String perHour = rate(row.perHour());
			context.drawText(font, Text.literal(value), names + COLUMN_GAP, y, color, true);
			if (!perHour.isEmpty()) {
				context.drawText(font, Text.literal(perHour),
						names + COLUMN_GAP + font.getWidth(value + "  "), y, 0xFFAAAAAA, true);
			}
			y += font.fontHeight;
		}
	}
